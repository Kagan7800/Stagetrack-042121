...
export interface MeetingActions {
  ...
  pushScreenVideo: (video: VideoState) => AnyAction;
  pullScreenVideo: () => AnyAction;
...
export const actions: MeetingActions = {
  ...
  pushScreenVideo(video) {
    return { type: 'PUSH_MEETING_SCREEN_SHARE_VIDEO', video };
  },
  pullScreenVideo() {
    return { type: 'PULL_MEETING_SCREEN_SHARE_VIDEO' };
  },
  pushCall(call) {
...
export function reducers(state = meeting, action: any) {
  switch (action.type) {
    ...
    case 'PUSH_MEETING_SCREEN_SHARE_VIDEO': {
      const isExists = some(meeting.videos, { kind: action.video.kind });
      if (!isExists) meeting.videos.push(action.video);
      return cloneDeep(meeting);
    }
    ...
    case 'PULL_MEETING_SCREEN_SHARE_VIDEO': {
       const video = find(meeting.videos, { kind:"screen" });
       if (video) meeting.videos = reject(meeting.videos, {  kind: 'screen' });
       return cloneDeep(meeting);
